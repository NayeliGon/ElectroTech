<footer class="text-center text-lg-start text-muted" style="background-color: #113946;">
  <section
    class="d-flex justify-content-center justify-content-lg-between p-4  border-bottom">

    <div class="me-3 d-none d-lg-block">
      <span>Siguenos en nuestras redes sociales:</span>
    </div>
        <a href="" class="me-4 text-reset">
          <i class="bi bi-facebook"></i>
        </a>
        <a href="" class="me-4 text-reset">
          <i class="bi bi-twitter"></i>
        </a>
        <a href="" class="me-4 text-reset">
          <i class="bi bi-google"></i>
        </a>
        <a href="" class="me-4 text-reset">
          <i class="bi bi-instagram"></i>
        </a>
        <a href="" class="me-4 text-reset">
          <i class="bi bi-linkedin"></i>
        </a>
    </div>
  </section>
    <div class="container text-center text-md-start mt- bg-dark">
      <div class="row mt-3">
        <div class="col-md-3 col-lg-4 col-xl-3 mx-auto mb-4">
          <h6 class="text-uppercase fw-bold mb-4">
            <i class="fas fa-gem me-3"></i>¡Conocenos!
          </h6>
          <p>
          Donde la tecnología se encuentra con la innovación. ¡Potenciamos tu mundo digital!
          </p>
        </div>
        <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mb-md-0 mb-4">
          <h6 class="text-uppercase fw-bold mb-4">
            Contactanos en:
          </h6>
          <p><i class="fas fa-home me-3"></i> Cobán Alta Verapaz zona 5</p>
          <p>
            <i class="fas fa-envelope me-3"></i>
            electrotechcoban@gmail.com
          </p>
          <p><i class="fas fa-phone me-3"></i> +502 89478845</p>
        </div>
      </div>
    </div>
    
  <div class="text-center bd-dark">
    © ElectroTech 2023: Grupo Toad
  </div>
  </section>


</footer>
  </body>